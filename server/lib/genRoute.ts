...
export default createRoute((app) => {
    ...
    const genDir = RPC_LIB_DIR + '/' + basename(app.config.client.path)

    console.log(`libHandler(`, [app.config.client.path, genDir], `)`)

    return {
        match(request, url) {
            return request.method === 'GET' && url.pathname.startsWith(app.config.client.base)
        },
        async fetch(request) {
            ...
            const srcFilePath = app.config.client.path + url.pathname
